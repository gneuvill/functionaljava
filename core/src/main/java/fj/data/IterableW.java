...
public final class IterableW<A> implements Iterable<A> {
  ...
  public <B> IterableW<B> apply(final Iterable<F<A, B>> f) {
    return wrap(f).bind((final F<A, B> f) -> map(f));
  }
  ...
  public static <A, B, C> F<Iterable<A>, F<Iterable<B>, IterableW<C>>> liftM2(final F<A, F<B, C>> f) {
    return curry((final Iterable<A> ca, final Iterable<B> cb) -> bind(ca, cb, f));
  }
  ...
  public static <A, T extends Iterable<A>> IterableW<IterableW<A>> sequence(final Iterable<T> as) {
    ...
    return ts.isEmpty() ? iterable(wrap(Option.<A>none())) : wrap(ts.head()).bind((F<A, Iterable<IterableW<A>>>) a -> sequence(ts.tail().map(IterableW.<T, Stream<T>>wrap())._1())
        .bind((final IterableW<A> as) -> iterable(wrap(Stream.cons(a, new P1<Stream<A>>() {
            public Stream<A> _1() {
  ...
  public static <A, B> F<F<A, B>, F<IterableW<A>, IterableW<B>>> map() {
    return f -> new F<IterableW<A>, IterableW<B>>() {
